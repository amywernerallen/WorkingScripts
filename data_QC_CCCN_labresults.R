...
library(RODBC)
myconn <- odbcConnect("CCCN_Warehouse_QA")
...
# Clean data
trig = sqlQuery(myconn, "select * from t_result where pat_id in (select enc_patient_id from t_encounter where enc_timestamp >= dateadd(day, 1, dateadd(month, -18 , '01-01-2014')) ) and lab_type ='trig'")
trig = sapply(trig, as.character)
trig[is.na(trig)] = " "
trig = as.data.frame(trig)

length(grep(pattern = ">", x = trig$result_value)) # None
length(grep(pattern = "<", x = trig$result_value)) # None
...
#### Look at results
# Max/Min values
max(trig$result_value_numeric[!is.na(trig$result_value_numeric)]) #354
min(trig$result_value_numeric[!is.na(trig$result_value_numeric)]) #-94 Negative values?
length(trig$result_value_numeric[trig$result_value_numeric <= 0]) #7 negatives
...
# Averages
mean(trig$result_value_numeric, na.rm=TRUE) #110.2912
median(hdl$result_value_numeric, na.rm=TRUE) #44
sd(hdl$result_value_numeric, na.rm=TRUE) #14.44478
...
# Normal range: 3-14%
trig_range = trig[trig$result_value_numeric < 300 & trig$result_value_numeric > 5,]
dim(trig_range) #19050
dim(trig)[1] #35919 total

dim(trig[trig$result_value_numeric == " ",])[1] #490 missing values
