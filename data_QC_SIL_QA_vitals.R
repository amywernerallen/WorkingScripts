...
height = sqlQuery(myconn, "select distinct vitals_height, vitals_height_unit, datediff(year, pat_date_of_birth, '01-01-2014') age from t_vitals vit inner join t_patient pat on vit.vitals_patient_id = pat.pat_id where vitals_patient_id in (select enc_patient_id from t_encounter where enc_timestamp >= dateadd(day, 1, dateadd(month, -18 , '01-01-2014')) ) ")
height = na.omit(height)
height$conversion = 0
dim(height)[1] # total number: 30433
...
dim(height[height$vitals_height_unit == "cm",]) #19349

# Some looks into the data:
height[height$conversion == max(height$conversion),] # what is max? 104 in
height[height$conversion == min(height$conversion),] # what is min? 0.33 in

mean(height$conversion, na.rm=TRUE) #46.7257
median(height$conversion, na.rm=TRUE) #44
sd(height$conversion, na.rm=TRUE) #14.44478

dim(height[height$conversion == 'NA',])

dim(height[height$conversion > 100,])[1] # how many are over 100 inches? 619
dim(height[height$conversion < 10 & height$age > 1,])[1] # how many are under 10 inches? 16
dim(height)[1] # total number: 723
dim(height[height$conversion < 100 & height$conversion > 10,])[1] 
...
height_temp[height_temp$age < 5 & height_temp$conversion > 60,]
...
dim(weight[weight$vitals_weight_unit == "kg",]) #61100

# Some looks into the data:
weight[weight$conversion == max(weight$conversion),] # what is max? 901126 lb
weight[weight$conversion == min(weight$conversion),] # what is min? 0 lb

mean(weight$conversion, na.rm=TRUE) #164.4957
median(weight$conversion, na.rm=TRUE) #161.3175
sd(weight$conversion, na.rm=TRUE) #14.44478

dim(height[height$conversion == 'NA',])

dim(weight[weight$conversion > 400,])[1] # how many are over 400 lbs? 464
dim(weight[weight$conversion < 2,])[1] # how many are under 2 lbs? 49
dim(weight)[1] # total number: 99978
dim(weight[weight$conversion < 400 & weight$conversion > 2,])[1]
...
BMI = sqlQuery(myconn, "select distinct vitals_BMI, datediff(year, pat_date_of_birth, '01-01-2014') age from t_vitals vit inner join t_patient pat on vit.vitals_patient_id = pat.pat_id where vitals_patient_id in (select enc_patient_id from t_encounter where enc_timestamp >= dateadd(day, 1, dateadd(month, -18 , '01-01-2014')) ) ")
BMI = na.omit(BMI)

# Some looks into the data:
BMI[BMI$vitals_BMI == max(BMI$vitals_BMI),] # what is max? 580012.6 
BMI[BMI$vitals_BMI == min(BMI$vitals_BMI),] # what is max? 580012.6 
...
systolic = sqlQuery(myconn, "select distinct vitals_systolic from t_vitals order by vitals_systolic")
systolic = na.omit(systolic)
diastolic = sqlQuery(myconn, "select distinct vitals_diastolic from t_vitals order by vitals_diastolic")
...
temperature = sqlQuery(myconn, "select distinct vitals_temperature, vitals_temperature_units from t_vitals order by vitals_temperature")
